...
export const useTheme = () => {
  const [theme, setTheme] = useState(configureStore.getState().settings.theme);
  ...
  useEffect(() => {
    const unsubscribe = configureStore.subscribe(() => {
      setTheme(configureStore.getState().settings.theme);
    });
    ...
    if (theme === 'systematic') {
      ...
      const listener = Appearance.addChangeListener(({ colorScheme }) => {
        if (colorScheme) {
          configureStore.dispatch(
            changeTheme(colorScheme === 'dark' ? 'dark' : 'light')
          );
        }
      });
